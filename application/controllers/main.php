...
class Main_Controller extends Template_Controller {
    ...
    public function index()
    {
		...
		*/
		...
		//untuk statistik
		$statistik_laporan = $this->repstat();
		$this->template->content->statistik_laporan = $statistik_laporan;
	}
	...
	public function repstat()
	{
		...
		//$data = "hello";
		$this->template->content->stas = new View('main/repstatistik');

		$this->themes->protochart_enabled = TRUE;
			$this->template->content->stas->failure = '';
			...
			$this->template->content->stas->range = $range;
			...
			$this->template->content->stas->category_data = $cats;
			...
			$this->template->content->stas->num_categories = $data['total_categories'];
			$this->template->content->stas->reports_per_cat = $reports_per_cat;

			$this->template->content->stas->reports_chart = $reports_chart->chart('reports',$report_data,$options,$colors,300,300);

			$this->template->content->stas->verified = 0;
			$this->template->content->stas->unverified = 0;
			$this->template->content->stas->approved = 0;
			$this->template->content->stas->unapproved = 0;
		...
		foreach ($data['verified_counts'] as $ver_or_un => $arr)
		{
		    ...
		    if ( ! isset($this->template->content->stas->$ver_or_un))
		    {
		        $this->template->content->stas->$ver_or_un = 0;
		    }
		    
		    foreach($arr as $count)
		    {
		        $report_staus_data[$ver_or_un][0] += $count;
		        $this->template->content->stas->$ver_or_un += $count;
		    }
		}
		
		$colors = array('verified'=>'0E7800','unverified'=>'FFCF00');
		$this->template->content->stas->report_status_chart_ver = $report_status_chart->chart('report_status_ver',$report_staus_data,$options,$colors,150,150);
		...
		$report_staus_data = array();
		    ...
		    if ( ! isset($this->template->content->stas->$app_or_un))
		    {
		        $this->template->content->stas->$app_or_un = 0;
		    }
		    
		    foreach($arr as $count)
		    {
		        $report_staus_data[$app_or_un][0] += $count;
		        $this->template->content->stas->$app_or_un += $count;
		    }
		}
		
		$this->template->content->stas->num_reports = $data['total_reports'];
		
		$colors = array('approved'=>'0E7800','unapproved'=>'FFCF00');
		$this->template->content->stas->report_status_chart_app = $report_status_chart->chart('report_status_app',$report_staus_data,$options,$colors,150,150);
		
		// Set the date
		$this->template->content->stas->dp1 = date('Y-m-d',$data['earliest_report_time']);
		$this->template->content->stas->dp2 = date('Y-m-d',$data['latest_report_time']);

		//$this->template->content->stas->data = $data;		
		return $this->template->content->stas;
	...
	//tambahan buat incident image 15 feb 2014 .K.
	
	public function halo()
	{
		$foto = array();
		$id = array();
		$insidens = Incident_Model::get_incidents();
		foreach( $insidens as $insiden ){
			$insiden = ORM::factory('incident', $insiden->incident_id);
			//echo $insiden."<br />";
		}
		/*
		...
		foreach($aksi as $aksis)
		{
			...
			//$id[] = $media->incident_id; //perbaikan penting 29 juli 2012
			print_r($aksis->media);
		}
