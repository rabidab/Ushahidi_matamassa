...
class reports_Core {
	...
	public static function validate(array & $post)
	{
		...
		{
			$post->add_rules('person_first', 'required' , 'length[2,100]'); //permintaan via whatsapp
		}
		else
		{
			$post->person_first = '';
			$post->add_error('person_first','required'); //permintaan via whatsapp
		}

		if ( ! empty($post->person_last))
		{
			$post->add_rules('person_last', 'required' , 'numeric' ,'length[2,13]'); //permintaan via whatsapp
		}
		else
		{
			$post->person_last = '';
			$post->add_error('person_last','required'); //permintaan via whatsapp
		}
